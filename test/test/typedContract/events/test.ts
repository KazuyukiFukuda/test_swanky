import type * as EventTypes from '../event-types/test';
...
export default class EventsClass {
	...
	public subscribeOnTransferEventEvent(callback : (event : EventTypes.TransferEvent) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('TransferEvent', 'test')) as EventTypes.TransferEvent);
		};
	...
	public subscribeOnApprovalEventEvent(callback : (event : EventTypes.ApprovalEvent) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('ApprovalEvent', 'test')) as EventTypes.ApprovalEvent);
		};
